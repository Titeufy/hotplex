...
func (a *Adapter) handleWebhook(w http.ResponseWriter, r *http.Request) {
	...
	if a.Adapter.Handler() != nil {
		reqCtx := r.Context()
		go func() {
			if err := a.Adapter.Handler()(reqCtx, msg); err != nil {
				a.Adapter.Logger().Error("Handle message failed", "error", err)
			}
		}()
	}
...
func (a *Adapter) Stop() error {
	if a.config.WebhookURL != "" {
		a.DeleteWebhook(context.Background())
	}
